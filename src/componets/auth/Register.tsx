...
export default function Register() {
    ...
    const handleRegister = async() => {
        try {
            const data: any = await fetchRegister(email, password);
            login(data.token)
            navigate('/');
        } catch (error) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen">
            ...
            <form
                ...
                className="w-[300px] flex flex-col gap-4 mt-4">
                <input
                    type="text"
                    required
                    value={email}
                ...
                <button
                    type="submit"
                    className="dark:bg-white bg-black text-white rounded-4xl dark:text-black font-semibold p-1.5 px-4 hover:opacity-90 transition-all duration-300 cursor-pointer">Registrate</button>
                <p className="text-[#666] text-sm">¿Ya tienes cuenta? <a href="/auth/login" className="text-black dark:text-white font-semibold">Inicia Sesion</a></p>
            </form>
        </div>
    )
}